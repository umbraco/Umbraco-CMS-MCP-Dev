import { putElementByIdPublishBody } from "@/umb-management-api/umbracoManagementAPI.zod.js";
import { z } from "zod";
...
const inputSchema = {
  id: z.string().uuid(),
  data: z.object(putElementByIdPublishBody.shape),
};

const PublishElementTool = {
  name: "publish-element",
  description: `Publishes an element by Id. IMPORTANT: If workflow approval is required, use the initiate-workflow-action function instead.
  ...
  handler: (async (model: { id: string; data: any }) => {
    ...
    return executeVoidApiCall((client) =>
      client.putElementByIdPublish(model.id, model.data, CAPTURE_RAW_HTTP_RESPONSE)
    );
  }),
} satisfies ToolDefinition<typeof inputSchema>;

export default withStandardDecorators(PublishElementTool);
