...
import {
  ...
  findBlockByKey,
  type BlockDataItem
} from "../../document/put/helpers/block-discovery.js";
import {
  type ToolDefinition,
  createToolResult,
  ToolValidationError,
...
export const updateElementBlockPropertyOutputSchema = z.object({
  success: z.boolean(),
  message: z.string(),
...
const updateElementBlockPropertySchema = {
  elementId: z.string().uuid().describe("The element containing the block"),
  propertyAlias: z.string().min(1).describe("Element property alias containing BlockList/BlockGrid"),
  culture: z.string().nullish().describe("Optional culture for variant element properties"),
  segment: z.string().nullish().describe("Optional segment for variant element properties"),
  updates: z.array(blockUpdateSchema).min(1).describe("Array of block updates"),
};

type UpdateElementBlockPropertyModel = {
  elementId: string;
...
const UpdateElementBlockPropertyTool = {
  name: "update-element-block-property",
  description: `Updates or adds property values within BlockList, BlockGrid, or RichText block content on an element.
  ...
  Example usage:
  - Update a block property: { elementId: "...", propertyAlias: "mainContent", updates: [{ contentKey: "block-uuid", blockType: "content", properties: [{ alias: "title", value: "New Title" }] }] }
  - Add a new property to block: { elementId: "...", propertyAlias: "mainContent", updates: [{ contentKey: "block-uuid", blockType: "content", properties: [{ alias: "newProp", value: "Value" }] }] }
  - Update with culture: { elementId: "...", propertyAlias: "mainContent", culture: "es-ES", updates: [...] }
  - Batch update multiple blocks: { elementId: "...", propertyAlias: "mainContent", updates: [{ contentKey: "uuid1", ... }, { contentKey: "uuid2", ... }] }`,
  inputSchema: updateElementBlockPropertySchema,
  outputSchema: updateElementBlockPropertyOutputSchema.shape,
  annotations: {
    idempotentHint: true,
  },
  slices: ['update'],
  enabled: (user: CurrentUserResponseModel) => user.fallbackPermissions.includes(UmbracoElementPermissions.Update),
  handler: (async (model: UpdateElementBlockPropertyModel) => {
    const client = UmbracoManagementClient.getClient();

    // Step 1: Fetch the current element
    const currentElement = await client.getElementById(model.elementId);

    // Step 2: Find the property containing the block structure
    const elementProperty = currentElement.values.find(v =>
      matchesProperty(v, model.propertyAlias, model.culture, model.segment)
    );

    if (!elementProperty) {
      const availableAliases = currentElement.values.map(v => ({
        alias: v.alias,
      ...
      throw new ToolValidationError({
        ...
        detail: `Property '${getPropertyKey(model.propertyAlias, model.culture, model.segment)}' does not exist on this element`,
        extensions: {
    ...
    const discoveredArrays = discoverAllBlockArrays(elementProperty.value, `property(${model.propertyAlias})`);
    ...
    if (discoveredArrays.length === 0) {
      throw new ToolValidationError({
        ...
        extensions: {
          propertyValue: elementProperty.value
        }
      });
    }
    ...
    if (notFoundBlocks.length === model.updates.length) {
      ...
      throw new ToolValidationError({
        ...
        detail: "None of the specified blocks were found in the element",
        extensions: {
          notFoundBlocks,
    ...
    // Step 7: Build the update payload (elements have no template)
    const updatePayload: UpdateElementRequestModel = {
      values: updatedValues,
      variants: variants,
    };

    // Step 8: Update the element
    await client.putElementById(model.elementId, updatePayload);
...
} satisfies ToolDefinition<typeof updateElementBlockPropertySchema, typeof updateElementBlockPropertyOutputSchema.shape>;

export default withStandardDecorators(UpdateElementBlockPropertyTool);
