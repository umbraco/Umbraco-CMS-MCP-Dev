...
3. Create an ELEMENT TYPE using create-element-type. Name it '_TestLibElementType', alias 'testLibElementType', icon from step 1, and set allowedInLibrary to true (required so library elements can be created from it). Add one property: name 'Heading', alias 'heading', dataTypeId set to the Textstring data type ID from step 2, tab 'Content'. Note the element type ID.
